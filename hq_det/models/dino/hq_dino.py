...
class HQDINO(HQModel):
    ...
    def compute_loss(self, batch_data, forward_result):
        ...
        targets = self._build_targets(batch_data)
        losses = self.criterion(outputs, targets, epoch=self.current_epoch)
        total_loss = sum(losses.values())
        info = {'loss': total_loss.item(), **{k: v.item() for k, v in losses.items()}}
