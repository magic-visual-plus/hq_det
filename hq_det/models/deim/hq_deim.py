...
class HQDEIM(HQModel):
    def __init__(self, class_id2names=None, **kwargs):
        ...
        if class_id2names is None:
            ...
            if 'model' in kwargs and kwargs['model']:
                ...
                if 'id2names' in data:
                    ...
                    self.id2names = {i: str(i) for i in range(80)}
            else:
                self.id2names = {i: str(i) for i in range(80)}
        else:
        ...
        # Build config path automatically if not provided
    ...
    def get_class_names(self):
        """Return list of class names in order of indices."""
    ...
    def postprocess(self, forward_result, batch_data, confidence=0.0):
        """Convert model output to PredictionResult using DEIM's postprocessor."""
        if 'origin_size' not in batch_data:
            # Fallback: use image size from 'img' tensor
        ...
        for i, pred in enumerate(preds):
            ...
            # Set image_id if available in targets
            if 'targets' in batch_data and i < len(batch_data['targets']):
                record.image_id = batch_data['targets'][i].get('image_id', 0)
            else:
                record.image_id = 0
    ...
    def imgs_to_batch(self, imgs):
        """Convert list of images (BGR uint8) to batch dict for inference."""
    ...
    def predict(self, imgs: List[np.ndarray], bgr=False, confidence=0.0, max_size=-1) -> List[PredictionResult]:
        ...
        max_size = self.image_size if max_size == -1 else max_size
        imgs_resized = []
        ...
        for i, img in enumerate(imgs):
            ...
            if max_hw > max_size:
                ...
                imgs_resized.append(cv2.resize(img, (int(img.shape[1] * rate), int(img.shape[0] * rate))))
                img_scales[i] = rate
            else:
                imgs_resized.append(img)

        with torch.no_grad():
            batch_data = torch_utils.batch_to_device(self.imgs_to_batch(imgs_resized), self.device)
