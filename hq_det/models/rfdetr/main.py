        ...
        if args.pretrain_weights is not None:
            ...
            if checkpoint_num_classes != args.num_classes + 1:
                logger.warning(
                    ...
                    f"reinitializing detection head with {args.num_classes} classes"
                )
                self.reinitialize_detection_head(args.num_classes + 1)  # HACK: 源码这里写的是checkpoint_num_classes, 但是实际是checkpoint_num_classes
                # HACK: 加载模型时, 需要将class_embed.weight和class_embed.bias裁剪到args.num_classes + 1
                for key in checkpoint['model'].keys():
                    if 'class_embed' in key:
                        checkpoint['model'][key] = checkpoint['model'][key][:args.num_classes + 1]
