...
DEFAULT_CONFIG_NAME = 'dino_swin_tiny_224_4scale_12ep'


class HQDINO_SWIN(HQModel):
    def __init__(self, class_id2names=None, **kwargs):
        super(HQDINO_SWIN, self).__init__(class_id2names, **kwargs)
        ckpt_path = kwargs.get('model')
        ...
        dino_swin_config_path = os.path.join(
            current_dir, 'configs', 'dino-swin', config_name)
        dino_swin_config = LazyConfig.load(dino_swin_config_path)
        # print(LazyConfig.to_py(dino_swin_config)) #查看配置结构
        # print(dino_swin_config)
        dino_swin_config.model.num_classes = len(self.id2names)
        # dino_swin_config.model['roi_heads']['box_predictor']['num_classes'] = len(self.id2names)
        # dino_swin_config.model['roi_heads']['mask_head']['num_classes'] = len(self.id2names)
        # dino_swin_config.model['roi_heads']['num_classes'] = len(self.id2names)
        # self.model = MODELS.build(dino_swin_config.model)
        self.model = instantiate(dino_swin_config.model)
        # patch detectron2 SwinTransformer typo: `_size_devisibility` -> `_size_divisibility`
        backbone = getattr(self.model, 'backbone', None)
        if backbone is not None and hasattr(backbone, '_size_devisibility') \
                and not hasattr(backbone, '_size_divisibility'):
            backbone._size_divisibility = backbone._size_devisibility
    ...
    def load_model(self, path):
        ...
        if not path or not os.path.exists(path):
            print(f'[HQDINO_SWIN] no checkpoint provided (path={path}), training from scratch')
            return
        ...
        # auto-prefix raw backbone-only checkpoints with `backbone.`
        if not any(k.startswith('backbone.') for k in state_dict) \
                and any(k.startswith('backbone.') for k in model_state):
            state_dict = {f'backbone.{k}': v for k, v in state_dict.items()}
        new_state_dict = {
            k: v for k, v in state_dict.items()
            if k in model_state and v.shape == model_state[k].shape
        }
        print(f'[HQDINO_SWIN] loaded {len(new_state_dict)}/{len(state_dict)} checkpoint keys, '
              f'{len(model_state) - len(new_state_dict)} model keys not loaded')
        missing = [k for k in model_state.keys() if k not in new_state_dict]
        if missing:
            print(f'[HQDINO_SWIN] missing keys (first 10): {missing[:10]}')
    ...
    def predict(self, imgs, bgr=False, confidence=0.0, max_size=-1) -> List[PredictionResult]:
        ...
        device = self.device
        with torch.no_grad():
            batch_data = HQDINO_SWIN.imgs_to_batch(imgs)
    ...
    def to(self, device):
        super(HQDINO_SWIN, self).to(device)
