_base_ = '../common/ssj_scp_270k_coco-instance.py' # HACK: 修改为本地配置

"""
本配置文件修改自 CO-DETR 的 co_dino_5scale_r50_lsj_8xb2_1x_coco.py
主要修改内容:
1. 修改了 _base_ 路径,指向本地的 ssj_scp_270k_coco-instance.py
2. 添加了 codetr 相关模块的导入和配置

所有修改过的模块均使用 HACK 标记, 以方便后续修改
"""

# HACK: 添加 codetr 模块路径, 加入的代码块用 '---' 包括
# ------------------------------------------------------------
import os
import sys
import inspect  
...
append_dir = os.path.abspath(os.path.join(current_dir, '..', '..', ))
print(append_dir)
sys.path.append(append_dir)

from codetr import (
    CoDETR, CoDINOHead, CoDinoTransformer, DetrTransformerEncoder, 
    DinoTransformerDecoder, DetrTransformerDecoderLayer,
)
...
num_dec_layer = 6
loss_lambda = 2.0
num_classes = int(os.environ.get('NUM_CLASSES', 80))    # HACK: 修改原有配置, 通过环境变量设置
...
model = dict(
    ...
    data_preprocessor=dict(
        type=DetDataPreprocessor,   # HACK: 原本为 'DetDataPreprocessor'
    ...
    neck=dict(
        ...
        num_outs=5),
    query_head=dict(
        type=CoDINOHead,  # HACK: 原本为 'CoDINOHead'
        num_query=900,
        ...
        transformer=dict(
            type=CoDinoTransformer,  # HACK: 原本为 'CoDinoTransformer'
            with_coord_feat=False,
            num_co_heads=2,  # ATSS Aux Head + Faster RCNN Aux Head
            num_feature_levels=5,
            encoder=dict(
                type=DetrTransformerEncoder,  # HACK: 原本为 'DetrTransformerEncoder'
                num_layers=6,
            ...
            decoder=dict(
                type=DinoTransformerDecoder,  # HACK: 原本为 'DinoTransformerDecoder'
                num_layers=6,
                return_intermediate=True,
                transformerlayers=dict(
                    type=DetrTransformerDecoderLayer,  # HACK: 原本为 'DetrTransformerDecoderLayer'
                    attn_cfgs=[
                        dict(
