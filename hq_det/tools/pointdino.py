"""Train and evaluate PointDINO with its complete MMEngine configuration.

The point predictions and FIDT loss require the detector's normal loss and
predict paths. These entry points therefore use MMEngine Runner directly.
"""
...
def _parse_args(argv=None, *, evaluation=False):
    ...
    parser.add_argument(
        '--data-root', help='ShanghaiTech Part-B root; updates all data loaders')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Distributed launcher (default: none)')
    parser.add_argument(
...
def train_main(argv=None):
    ...
    return Runner.from_cfg(cfg).train()
...
def evaluate_main(argv=None):
    ...
    return Runner.from_cfg(cfg).test()
...
def _dataset_config(cfg, name, data_path, ann_file, image_dir, class_names,
    ...
    if not isinstance(content, dict) or 'data_list' not in content:
        raise ValueError(f'{annotation}: expected PointDINO JSON with metainfo '
                         'and data_list. See scripts/POINTDINO_DATA_FORMAT.md.')
    declared = content.get('metainfo', {}).get('classes')
    classes = class_names if class_names is not None else declared
    if (not isinstance(classes, (list, tuple)) or not classes
            or not all(isinstance(item, str) and item for item in classes)):
        raise ValueError(f'{annotation}: metainfo.classes must contain class names.')
    classes = list(classes)
    if len(set(classes)) != len(classes):
        raise ValueError(f'{annotation}: class names must be unique.')
    if declared is not None and list(declared) != list(classes):
        raise ValueError(f'{annotation}: class order differs between splits '
                         'or from class_names.')
...
def build_run_config(
        ...
        train_ann_file='train_point.json', val_ann_file='test_point.json',
        test_ann_file=None, train_image_dir='train_data/images',
        val_image_dir='test_data/images', test_image_dir=None, class_names=None,
    ...
    Class IDs follow the order in JSON ``metainfo.classes``. Full-model
    ...
    if evaluation:
        ...
        _dataset_config(cfg, 'val_dataloader', data_path, val_ann_file,
                        val_image_dir, classes, image_size, eval_batch_size,
                        num_data_workers)
        ...
        if test_ann_file is not None:
            _dataset_config(
                ...
                val_image_dir if test_image_dir is None else test_image_dir,
                classes, image_size, eval_batch_size, num_data_workers)
...
def _select_devices(devices):
    if devices is not None:
        if len(devices) > 1 and int(os.environ.get('WORLD_SIZE', '1')) == 1:
            raise ValueError('For multiple devices, launch this script with '
                             'torchrun --nproc_per_node=<GPU count>.')
