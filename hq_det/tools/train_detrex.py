import sys
import os
from hq_det.models import detrex
...
class MyTrainer(HQTrainer):
    ...
    def build_model(self):
        ...
        model = detrex.HQDetrex(class_id2names=id2names, **self.args.model_argument)
        return model
...
def run(data_path, output_path, num_epoches, lr0, load_checkpoint, eval_class_names=None, 
        batch_size=6, image_size=1024, gradient_update_interval=1, lr_backbone_mult=0.1, 
        num_data_workers=16, checkpoint_name='ckpt.pth', devices=[0], config_path=None,
        warmup_epochs=0, project='dino', model_size='r50', model_name=None):
    """
    Run Detrex training
    
    Args:
        project: Model project, 'dino', 'detr', 'deformable_detr', etc. (default: 'dino')
        model_size: Model size:
            - For dino with ResNet: 'r50', 'r101' (default: 'r50')
            - For dino with Swin: 'swin_tiny_224', 'swin_small_224', 'swin_large_384', etc.
            - For dino with ConvNext: 'convnext_tiny_384', 'convnext_small_384', etc.
        model_name: Full model config name, e.g., 'dino_r50_4scale_12ep' (overrides above)
    ...
    if config_path:
        ...
        model_argument["project"] = project
